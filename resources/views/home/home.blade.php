...
<div class="legal-knowledge-categories-section">
    <div class="legal-knowledge-categories-container">
        ...
        <div class="legal-knowledge-categories-grid" data-aos="fade-up">
            ...
            <a href="{{ route('frontend.legalknowledgelibrary') }}" style="margin-bottom:15px; padding:16px 18px; border:2px solid #ffe8dd; border-left:4px solid #ff5722; border-radius:8px; background:linear-gradient(135deg, #fff9f5 0%, #ffffff 100%); box-shadow: 0 2px 8px rgba(255,87,34,0.08); transition: all 0.3s ease; text-decoration:none; display:flex; align-items:center; gap:14px;" data-aos="fade-up">
                <div style="font-size:28px; width:48px; height:48px; background:rgba(255,87,34,0.15); border-radius:8px; display:flex; align-items:center; justify-content:center; flex-shrink:0; color:#ff5722;">
                    <i class="fa-solid {{ $legalKnowledgeIcons[$lkCategory->name] ?? 'fa-scale-balanced' }}"></i>
                </div>
                <div style="flex:1; min-width:0;">
                    <h3 style="font-size:15px; font-weight:700; color:#1f2937; margin:0 0 4px 0; font-family:'Poppins',sans-serif; overflow:hidden; text-overflow:ellipsis; display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical;">{{ $lkCategory->name }}</h3>
                    <span style="font-size:12px; font-weight:600; color:#ff5722; font-family:'Poppins',sans-serif; display:block; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">Explore <i class="fa-solid fa-arrow-right" style="margin-left:4px;"></i></span>
                </div>
            </a>
            @empty
            <div class="no-content-message" style="grid-column: 1 / -1;">
        ...
        <div class="courses-notes-grid" data-aos="fade-up">
            ...
            <div style="margin-bottom:15px; padding:18px 20px; border:2px solid #ffe8dd; border-left:4px solid #ff5722; border-radius:8px; background:linear-gradient(135deg, #fff9f5 0%, #ffffff 100%); box-shadow: 0 2px 8px rgba(255,87,34,0.08); transition: all 0.3s ease;">
                <div style="display:flex; align-items:flex-start; gap:12px;">
                    ...
                    <div style="flex:1;">
                        <div style="font-weight:700; color:#1f2937; margin-bottom:8px; font-size:15px;">
                            {{ Str::limit($legalKnowledgeNote->description ?? 'Legal Knowledge', 60) }}
                        </div>
                        @if(count($legalKnowledgeNote->pdfs ?? []) > 0)
                        <div style="margin-top:8px; display:flex; justify-content:space-between; align-items:center; padding:8px 0; border-top:1px solid #f0f0f0;">
                            <span style="font-size:13px; color:#666;"><i class="fa-solid fa-file-pdf" style="color:#ff5722; margin-right:6px;"></i>{{ count($legalKnowledgeNote->pdfs ?? []) }} PDF{{ count($legalKnowledgeNote->pdfs ?? []) === 1 ? '' : 's' }} available</span>
                            <a href="{{ route('frontend.legalknowledgelibrary') }}" style="font-size:12px; color:#ff5722; font-weight:600; text-decoration:none; transition: all 0.3s ease;">View All →</a>
                        </div>
                        @endif
                    </div>
                </div>
