...
<style>
    ...
    /* ===== MODERN ACCORDION/CARD DESIGN ===== */
    .legalknowledge-category, .accordion-content, [class*="accordion"] {
    ...
    [class*="legalknowledge"], [class*="accordion"] {
        color: #d4af37 !important;
    }

    [class*="legalknowledge"] div, [class*="accordion"] div {
    ...
    <div class="container">
        <div class="row">
            ...
            <div style="width:100%; max-width:1100px; margin:auto;">
                ...
                <div style="background:linear-gradient(135deg, #f9fafb 0%, #ffffff 100%); padding:30px 25px; border-radius:12px;
                    ...
                    <div style="text-align:center; margin-bottom:20px;">
                        <h3 style="font-size:18px; font-weight:700; color:#1f2937; margin:0 0 8px 0;">
                            <i class="fa-solid fa-sliders" style="margin-right:8px; color:#128C7E;"></i>Find Your Legal Knowledge Note
                        </h3>
                    ...
                    <div style="display:grid; grid-template-columns:1fr 1fr; gap:20px; margin-bottom:15px;">
                        ...
                        <div class="category-filter-wrapper" style="position:relative;">
                            <label style="display:block; font-size:12px; font-weight:700; color:#374151; margin-bottom:8px; text-transform:uppercase; letter-spacing:0.5px;">
                                <i class="fa-solid fa-filter" style="margin-right:6px;"></i>Category
                            </label>
                            <div style="position:relative;">
                                ...
                                <div id="categoryDropdownMenu"
                                    ...
                                    <div style="max-height:380px; overflow-y:auto;">
                                        ...
                                        <div class="dropdown-item-legalknowledge" data-category-id="all"
                                            ...
                                            <i class="fa-solid fa-list" style="margin-right:8px;"></i>All Legal Knowledge
                                        </div>

                                        @foreach ($categories as $category)
                                        <!-- Parent Category -->
                                        <div class="dropdown-parent-legalknowledge" data-category-id="{{ $category->id }}"
                                            style="padding:14px 16px; cursor:pointer; border-bottom:1px solid #f3f4f6;
                                                    display:flex; justify-content:space-between; align-items:center;
                        ...
                        <div class="search-container" style="position:relative;">
                            ...
                            <div style="position:relative;">
                                <input type="text" id="noteSearch" class="form-control"
                                    placeholder="Search Legal Knowledge..." onkeyup="searchNotes(this.value)"
                ...
                @foreach ($categories as $category)
                <!-- CATEGORY -->
                <div class="legalknowledge-category" data-category-id="{{ $category->id }}" style="margin-bottom:15px; border:1px solid #ddd; border-radius:10px; overflow:hidden;">
                    ...
                    <div id="cat{{ $category->id }}" class="accordion-content" style="max-height:1000px;">
                        ...
                        <div style="margin:10px; border:1px dashed #ccc; border-radius:6px; overflow:hidden;">
                            ...
                            <div id="sub{{ $sub->id }}" class="accordion-content" style="padding:10px; max-height:1000px;">
                                ...
                                <!-- LEGAL KNOWLEDGE ENTRY -->
                                <div data-legalknowledge-id="{{ $note->id }}" style="margin-bottom:10px; padding:10px; border:1px solid #eee; border-radius:6px;">
                                    ...
                                    <!-- PDFs -->
                                    @if ($note->pdfs)
                                    @foreach ($note->pdfs as $index => $pdf)
            ...
            <style>
                ...
                .dropdown-item-legalknowledge:hover,
                .dropdown-parent-legalknowledge:hover {
                    background: linear-gradient(135deg, #10b981 0%, #059669 100%) !important;
                    color: #fff !important;
                }

                .dropdown-item-legalknowledge:hover i,
                .dropdown-parent-legalknowledge:hover i {
                    color: #fff !important;
                }
            ...
            <!-- SCRIPT -->
            <script>
                let selectedLegalKnowledgeCategoryId = 'all';
                ...
                document.querySelectorAll('.dropdown-parent-legalknowledge').forEach(item => {
                    item.addEventListener('click', function() {
                        const categoryId = this.dataset.categoryId;
                        const categoryName = this.querySelector('span').innerText.replace(/[^\w\s-]/g, '').trim();

                        selectedLegalKnowledgeCategoryId = categoryId;
                        ...
                        // Filter
                        filterLegalKnowledgeByCategory(categoryId);
                    });
                });

                // Handle "All Categories" Option
                document.querySelector('.dropdown-item-legalknowledge[data-category-id="all"]').addEventListener('click', function() {
                    selectedLegalKnowledgeCategoryId = 'all';
                    ...
                    filterLegalKnowledgeByCategory('all');
                });

                // Dropdown Item Hover Effects
                document.querySelectorAll('.dropdown-item-legalknowledge, .dropdown-parent-legalknowledge').forEach(item => {
                    item.addEventListener('mouseenter', function() {
                        this.style.background = '#e8f5f3';
                    });

                    item.addEventListener('mouseleave', function() {
                        if (item.classList.contains('dropdown-item-legalknowledge')) {
                ...
                // Filter legal knowledge entries by category
                function filterLegalKnowledgeByCategory(categoryId) {
                    const categoryCards = document.querySelectorAll('.legalknowledge-category');
                ...
                function searchNotes(query) {
                    ...
                    fetch(`{{ route('frontend.legalknowledgelibrarysearch') }}?q=${encodeURIComponent(query)}`)
                        .then(res => res.json())
                        .then(data => {
                            if (!data.length) {
                ...
                function openSearch(catId, subId, noteId) {
                    // Hide all categories
                    document.querySelectorAll('[id^="cat"]').forEach(cat => {
                        cat.parentElement.style.display = 'none'; // Hide the entire category container
                        cat.querySelectorAll('.legalknowledge-highlight').forEach(r => r.classList.remove('legalknowledge-highlight'));
                    });
                    ...
                    if (sub) {
                        sub.querySelectorAll('[data-legalknowledge-id]').forEach(r => r.style.display = 'none');

                        // Show only the searched entry
                        let entryDiv = sub.querySelector(`div[data-legalknowledge-id='${noteId}']`);
                        if (entryDiv) {
                            entryDiv.style.display = 'block';
                            entryDiv.classList.add('legalknowledge-highlight');
